...
async def call_grok(
    ...
    """Call xAI Grok API asynchronously via OpenAI-compatible endpoint."""
    if not XAI_API_KEY:
        return (
            "⚠️ *Grok AI is not configured.*\n\n"
            "The `XAI_API_KEY` environment variable is missing. "
            "Please add it in Railway → Variables to enable Grok AI."
        )
    try:
        messages = []
        if system_instruction:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if resp.status == 401:
                    return "⚠️ Grok API key is invalid. Please check your `XAI_API_KEY`."
                if resp.status == 429:
                    return "⚠️ Grok rate limit reached. Please try again in a moment."
                if resp.status != 200:
                    err = await resp.text()
                    logger.error(f"Grok API error {resp.status}: {err[:200]}")
                    return f"⚠️ Grok API error ({resp.status}). Please try again."
                data = await resp.json()
                return data["choices"][0]["message"]["content"]

    except asyncio.TimeoutError:
        return "⚠️ Grok request timed out. Please try again."
    except Exception as e:
        logger.error(f"Grok API error: {e}")
        return f"⚠️ Grok Error: {str(e)[:200]}"


def get_grok_history(user_id: int) -> list:
    """Get Grok-specific conversation history (separate from Gemini)."""
    return fb_get(f"grok_conversations/{user_id}") or []


def save_grok_history(user_id: int, history: list) -> None:
    """Save Grok-specific conversation history."""
    trimmed = history[-CONVERSATION_HISTORY_LIMIT:]
    fb_set(f"grok_conversations/{user_id}", trimmed)


def clear_grok_conversation(user_id: int) -> None:
    """Clear Grok conversation history."""
    fb_delete(f"grok_conversations/{user_id}")
...
async def ai_chat_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    _menu_buttons = {
        ...
        "🧮 Math", "🧠 Quiz", "🎁 Refer", "👤 Profile", "⚙️ Settings", "😋 Grok AI",
    }
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    elif data == "grok_clear_history":
        clear_grok_conversation(user.id)
        try:
            await query.edit_message_text(
                "✅ Grok chat history cleared! Start a fresh Grok conversation.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("😋 Back to Grok", callback_data="home")
                ]]),
            )
        except Exception:
            pass
...
@require_not_banned
@require_not_maintenance
async def grok_chat_entry(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Entry point for Grok AI Chat."""
    if not XAI_API_KEY:
        await update.effective_message.reply_text(
            "⚠️ *Grok AI Not Available*\n\n"
            "`XAI_API_KEY` is not configured on this server.\n\n"
            "Please ask the admin to add the Grok API key to enable this feature.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("🗑️ Clear History", callback_data="grok_clear_history"),
         InlineKeyboardButton("❌ Exit Chat", callback_data="home")],
    ])
    await update.effective_message.reply_text(
        "😋 *Grok AI Chat*\n\n"
        "You're now chatting with *Grok* — xAI's advanced language model!\n\n"
        "• Powered by xAI's Grok\n"
        "• Separate session from Gemini AI\n"
        "• Ask anything — studies, code, questions!\n\n"
        "💬 Type your message below:",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=kb,
    )
    return STATE_GROK_CHAT


@require_not_banned
@rate_limited
async def grok_chat_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle messages in Grok AI chat session."""
    user = update.effective_user
    user_text = update.message.text.strip()

    # Exit if user pressed a reply keyboard button
    _menu_buttons = {
        "🏠 Home", "🤖 AI", "📚 Study", "📄 PDF", "📝 Notes",
        "🧮 Math", "🧠 Quiz", "🎁 Refer", "👤 Profile", "⚙️ Settings", "😋 Grok AI",
    }
    ...
    history = get_grok_history(user.id)

    system_prompt = (
        "You are Grok, an AI assistant made by xAI. You are helping students learn and study. "
        "Be helpful, clear, and educational. Provide structured explanations with examples. "
        "For math, show step-by-step solutions. For code, add comments. "
        "Use emojis appropriately to make responses engaging."
    )
    ...
    history.append({"role": "user", "text": user_text})
    history.append({"role": "model", "text": response})
    save_grok_history(user.id, history)
    increment_stat(user.id, "ai_calls")
    add_history(user.id, "grok_chat", user_text[:100])
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📌 Save as Note", callback_data="save_note_ai"),
         InlineKeyboardButton("🗑️ Clear Chat", callback_data="grok_clear_history")],
        [InlineKeyboardButton("🏠 Home", callback_data="home"),
         InlineKeyboardButton("❌ Close", callback_data="close")],
    ])
...
async def reply_keyboard_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if text == "🏠 Home":
        ...
        return await grok_chat_entry(update, context)
...
def build_app() -> Application:
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(
                filters.Regex(
                    r"^(🏠 Home|🤖 AI|📚 Study|📄 PDF|📝 Notes|🧮 Math|🧠 Quiz|🎁 Refer|👤 Profile|😋 Grok AI|⚙️ Settings)$"
                ),
        ...
        states={
            ...
            STATE_GROK_CHAT: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, grok_chat_message),
                CallbackQueryHandler(callback_router),
            ],
        },
        fallbacks=[
            ...
            MessageHandler(
                filters.Regex(r"^(🏠 Home|🤖 AI|📚 Study|📄 PDF|📝 Notes|🧮 Math|🧠 Quiz|🎁 Refer|👤 Profile|😋 Grok AI|⚙️ Settings)$"),
                reply_keyboard_router,
            ),
