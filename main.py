...
def main_keyboard() -> ReplyKeyboardMarkup:
    ...
    buttons = [
        ...
        [KeyboardButton("📚 Study"), KeyboardButton("🧠 Quiz")],
        [KeyboardButton("📝 Notes"), KeyboardButton("📄 PDF")],
        [KeyboardButton("🧮 Math"), KeyboardButton("🌐 Translate")],
...
async def home_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    inline_kb = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📄 PDF", callback_data="menu_pdf"),
         InlineKeyboardButton("🧮 Math", callback_data="menu_math")],
        [InlineKeyboardButton("🌐 Translate", callback_data="menu_translate"),
         InlineKeyboardButton("👤 Profile", callback_data="menu_profile")],
    ])

    await update.message.reply_text(text, parse_mode=ParseMode.MARKDOWN, reply_markup=inline_kb)
...
async def ai_chat_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_text in ("🏠 Home", "/start", "/home"):
...
async def profile_premium_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_text(
        ...
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📩 Contact Admin", url=f"[t.me](https://t.me/{CHANNEL_USERNAME.lstrip()'@')}" if CHANNEL_USERNAME else "[t.me](https://t.me/)")],
            *nav_inline("menu_profile"),
        ]),
    )
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "home":
        ...
        try:
            await query.edit_message_text(text, parse_mode=ParseMode.MARKDOWN, reply_markup=inline_kb)
        except Exception:
            await query.message.reply_text(text, parse_mode=ParseMode.MARKDOWN, reply_markup=inline_kb)
        context.user_data.clear()
        return ConversationHandler.END
...
def build_app() -> Application:
    ...
    conv_handler = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(
                filters.Regex(
                    r"^(🏠 Home|🤖 AI Chat|📚 Study|📝 Notes|📄 PDF|🧮 Math|🧠 Quiz|🌐 Translate|👤 Profile|⚙ Settings)$"
                ),
