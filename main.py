...
def register_user(user_id: int, username: str, full_name: str, referrer_id: Optional[int] = None) -> dict:
    ...
    if not existing:
        ...
        current = firebase_db.reference("stats/global/total_users").get() or 0
        fb_update("stats/global", {"total_users": current + 1})
        if referrer_id:
...
def main_keyboard() -> ReplyKeyboardMarkup:
    ...
    buttons = [
        [KeyboardButton("🏠 Home")],
        [KeyboardButton("🤖 AI"), KeyboardButton("🧮 Math")],
        [KeyboardButton("📄 PDF"), KeyboardButton("📚 Study")],
        [KeyboardButton("📝 Notes"), KeyboardButton("🧠 Quiz")],
        [KeyboardButton("🎁 Refer"), KeyboardButton("👤 Profile")],
...
def study_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("❓ MCQ Generator", callback_data="study_mcq"),
...
def pdf_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        [InlineKeyboardButton("📋 PDF Summary", callback_data="pdf_summarize"),
         InlineKeyboardButton("❓ PDF → MCQ", callback_data="pdf_mcq")],
        [InlineKeyboardButton("📝 PDF → Notes", callback_data="pdf_notes"),
         InlineKeyboardButton("💬 Ask PDF", callback_data="pdf_ask")],
        [InlineKeyboardButton("📄 Extract Text", callback_data="pdf_extract"),
         InlineKeyboardButton("📷 OCR (Image→Text)", callback_data="ai_ocr")],
        *nav_inline("home"),
    ]
    return InlineKeyboardMarkup(buttons)
...
def ai_keyboard() -> InlineKeyboardMarkup:
    buttons = [
        ...
        [InlineKeyboardButton("💡 Explain Topic", callback_data="ai_explain"),
         InlineKeyboardButton("✏️ Grammar Fix", callback_data="ai_grammar")],
        [InlineKeyboardButton("📝 Essay Writer", callback_data="ai_essay"),
         InlineKeyboardButton("💻 Code Assistant", callback_data="ai_code")],
        [InlineKeyboardButton("🔁 Text Summary", callback_data="ai_summarize"),
...
async def send_long_message(
    ...
    """Send potentially long message in chunks, appending signature to last chunk."""
    chunks = split_long_message(text)
    for i, chunk in enumerate(chunks):
        is_last = i == len(chunks) - 1
        send_text = chunk + FOOTER_SIGNATURE if is_last else chunk
        try:
            if is_last:
                await message.reply_text(send_text, parse_mode=parse_mode, reply_markup=reply_markup)
            else:
                await message.reply_text(send_text, parse_mode=parse_mode)
        except TelegramError as e:
            logger.warning(f"Send error (trying plain text): {e}")
            try:
                clean = re.sub(r"[*_`\[\]()~>#+=|{}.!\\]", "", send_text)
                if is_last:
...
async def ai_chat_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    _menu_buttons = {
        "🏠 Home", "🤖 AI", "📚 Study", "📄 PDF", "📝 Notes",
        "🧮 Math", "🧠 Quiz", "🎁 Refer", "👤 Profile", "⚙️ Settings",
    }
...
async def reminder_time_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not re.match(r"^\d{2}:\d{2}$", text):
        await update.message.reply_text(
            "⚠️ Invalid format. Please send the time as *HH:MM* (e.g., `08:00`).",
            parse_mode=ParseMode.MARKDOWN,
        )
        return STATE_REMINDER_TIME
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠️ Invalid time. Please use *HH:MM* with valid hours (00–23) and minutes (00–59).",
            parse_mode=ParseMode.MARKDOWN,
        )
        return STATE_REMINDER_TIME

    # Save reminder to Firebase
    fb_set(f"reminders/{user.id}", {
        "time": text,
    ...
    await update.message.reply_text(
        f"✅ *Reminder Set!*\n\n"
        f"🔔 I'll remind you to study every day at *{text} UTC*.\n\n"
...
async def daily_reminder_job(context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send the daily study reminder message."""
    job = context.job
    user_name = (job.data or {}).get("user_name", "Student")
    messages = [
        f"📚 Hey {user_name}! Time to study — keep that streak alive! 🔥",
        f"🎓 {user_name}, your daily study session is waiting! Let's go! 🚀",
        f"⏰ Study reminder, {user_name}! A little every day goes a long way. 💡",
        f"🧠 Hi {user_name}! Don't break the chain — study a bit today! ✨",
        f"📖 {user_name}, your future self will thank you. Study time! 🏆",
    ]
    await context.bot.send_message(
        chat_id=job.chat_id,
        text=random.choice(messages) + FOOTER_SIGNATURE,
        parse_mode=ParseMode.MARKDOWN,
    )


async def restore_reminders(application: Application) -> None:
    """Restore all saved reminders from Firebase on bot startup."""
    try:
        reminders = fb_get("reminders") or {}
        count = 0
        for uid, data in reminders.items():
            ...
            if not time_str or not chat_id:
                continue
            try:
                ...
                logger.warning(f"Failed to restore reminder for {uid}: {e}")
        logger.info(f"✅ Restored {count} study reminder(s)")
    except Exception as e:
...
async def quiz_weekly_flow(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    weekly_topics = [
        "Science", "History", "Geography", "Technology", "Literature",
        "Mathematics", "Art & Culture", "Economics", "Biology", "Physics",
        "World Events", "General Knowledge",
    ]
    topic = weekly_topics[datetime.now().isocalendar()[1] % len(weekly_topics)]
    context.user_data["quiz_topic"] = topic
    context.user_data["quiz_type"] = "weekly"
    await update.effective_message.reply_text(
        f"📆 *Weekly Challenge*\n\nThis week's topic: *{topic}*\n\n"
        f"🔥 Weekly format: *10 questions* across a broad range — good luck!\n\nGenerating questions...",
        parse_mode=ParseMode.MARKDOWN,
    )
    return await generate_quiz(update, context, topic, num_questions=10)
...
async def generate_quiz(
    update: Update, context: ContextTypes.DEFAULT_TYPE, topic: str, num_questions: int = 5
) -> int:
...
async def profile_premium_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status_text = "✅ *You are a Premium member!*" if is_premium else "🔓 *You are on the Free plan.*"

    text = (
        f"💎 *Premium Plan*\n\n"
        f"{status_text}\n\n"
        f"🚀 *Unlock the full power of AI Study Bot:*\n\n"
        f"✅ Unlimited AI chat messages\n"
        f"✅ Advanced PDF analysis (larger files)\n"
        f"✅ Unlimited quiz attempts & history\n"
        f"✅ Unlimited notes storage\n"
        f"✅ Priority AI response speed\n"
        f"✅ Personalized AI study plans\n"
        f"✅ Advanced progress analytics\n"
        f"✅ Early access to new features\n\n"
        f"💰 *How to upgrade:*\n"
        f"Contact the admin directly to get Premium access.\n\n"
        f"📩 Tap the button below to reach out!"
    )

    # Build contact button: prefer channel username, else direct admin link
    if CHANNEL_USERNAME:
        contact_url = f"https://t.me/{CHANNEL_USERNAME.lstrip('@')}"
        btn_label = f"📩 Contact via @{CHANNEL_USERNAME.lstrip('@')}"
    else:
        contact_url = f"https://t.me/{context.bot.username}"
        btn_label = "📩 Contact Admin"

    await update.effective_message.reply_text(
        text,
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton(btn_label, url=contact_url)],
            *nav_inline("menu_profile"),
        ]),
    )
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "home":
        ...
        try:
            ...
            pass
        await query.message.reply_text("👇 Select a feature:", reply_markup=main_keyboard())
        context.user_data.clear()
        return ConversationHandler.END
    ...
    # ── Study callbacks ──
        ...
        existing = fb_get(f"reminders/{user.id}/time")
        existing_text = f"\n\n⏰ *Current reminder:* `{existing}`" if existing else ""
        await query.edit_message_text(
            "🔔 *Study Reminder*\n\n"
            "Set a daily reminder and I'll message you every day at that time to study!\n\n"
            "Send the time in *HH:MM* format (24-hour, UTC).\n"
            "Examples: `07:30`, `18:00`, `21:15`"
            f"{existing_text}",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🗑️ Remove Reminder", callback_data="reminder_remove")],
                *nav_inline("menu_study"),
            ]),
        )
        return STATE_REMINDER_TIME

    elif data == "reminder_remove":
        fb_delete(f"reminders/{user.id}")
        # Remove any existing job for this user
        current_jobs = context.job_queue.get_jobs_by_name(f"reminder_{user.id}")
        for job in current_jobs:
            job.schedule_removal()
        await query.edit_message_text(
            "✅ *Reminder removed.*\n\nYou won't receive daily study reminders anymore.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(nav_inline("menu_study")),
        )
        return ConversationHandler.END
        ...
        pdfs = user_data.get("pdfs_processed", 0)
        text = (
            ...
            f"📝 Notes Created: {notes}\n"
            f"📄 PDFs Processed: {pdfs}\n\n"
...
def build_app() -> Application:
    ...
    conv_handler = ConversationHandler(
        ...
        entry_points=[
            ...
            MessageHandler(
                filters.Regex(
                    r"^(🏠 Home|🤖 AI|📚 Study|📄 PDF|📝 Notes|🧮 Math|🧠 Quiz|🎁 Refer|👤 Profile|⚙️ Settings)$"
                ),
        ...
        fallbacks=[
            ...
            MessageHandler(
                filters.Regex(r"^(🏠 Home|🤖 AI|📚 Study|📄 PDF|📝 Notes|🧮 Math|🧠 Quiz|🎁 Refer|👤 Profile|⚙️ Settings)$"),
                reply_keyboard_router,
            ),
